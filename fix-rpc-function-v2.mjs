...
async function fixRPCFunction() {
  ...
  console.log('=================================');
  console.log('  修复 RPC 函数 is_promoted 歧义');
  console.log('=================================\n');
  ...
  try {
    ...
    const checkResult = await pool.query(`
      SELECT EXISTS (
        SELECT FROM pg_proc 
        WHERE proname = 'get_square_works_with_promotion'
      );
    `);
    
    if (!checkResult.rows[0].exists) {
      console.log('⚠️ 函数 get_square_works_with_promotion 不存在，需要创建');
    } else {
      console.log('✅ 函数 get_square_works_with_promotion 存在\n');
    }
    ...
    await pool.query(`
      ...
      BEGIN
        ...
        SELECT 
          ...
          combined.creator_avatar,
          combined.views,
          combined.likes,
          combined.comments,
          combined.created_at,
          combined.is_promoted,
          combined.promoted_work_id,
          combined.package_type,
          combined.promotion_weight
        FROM (
          -- 推广作品
          SELECT
    ...
    await pool.query(`
      ...
      IS '获取广场作品列表，包含推广作品（修复 is_promoted 歧义版v2）';
    `);
