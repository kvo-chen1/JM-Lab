...
if (!supabaseServiceKey) {
  ...
  console.log('');
  console.log('使用方法:');
  console.log('  set SUPABASE_SERVICE_ROLE_KEY=your_key_here && node scripts/apply-scoring-tables-migration.mjs');
  process.exit(1);
}
...
const migrationPath = join(__dirname, '..', 'supabase', 'migrations', '20260223000006_create_missing_scoring_tables.sql');
    ...
    for (let i = 0; i < statements.length; i++) {
      ...
      if (error) {
        ...
        if (error.message.includes('does not exist') && 
            (statement.includes('DROP TABLE') || statement.includes('DROP VIEW') || statement.includes('DROP TRIGGER'))) {
          console.log('   ⚠️  对象不存在，跳过删除操作');
          continue;
        }
        // 如果是 "已存在" 错误，继续执行
