-- ============================================
-- 津门老字号文创商城与IP孵化品牌授权系统 - 数据库迁移 (Neon PostgreSQL)
-- 创建时间: 2026-03-07
-- ============================================
...
-- ============================================
-- 2. 品牌授权申请表 (brand_authorizations)
-- ============================================
CREATE TABLE IF NOT EXISTS brand_authorizations (
  ...
  proposed_usage TEXT,
  proposed_duration INTEGER, -- 授权期限（月）
...
CREATE TABLE IF NOT EXISTS products (
  ...
  status VARCHAR(20) DEFAULT 'pending', -- pending, approved, on_sale, off_shelf, sold_out
...
CREATE TABLE IF NOT EXISTS orders (
  ...
  status VARCHAR(20) DEFAULT 'pending_payment', -- pending_payment, paid, shipped, completed, cancelled, refunded, refunding
...
BEGIN
  ...
  IF NOT EXISTS (SELECT 1 FROM pg_trigger WHERE tgname = 'update_brand_authorizations_updated_at') THEN
    CREATE TRIGGER update_brand_authorizations_updated_at
      BEFORE UPDATE ON brand_authorizations
      FOR EACH ROW EXECUTE FUNCTION update_updated_at_column();
  END IF;
  ...
  -- shopping_carts
  IF NOT EXISTS (SELECT 1 FROM pg_trigger WHERE tgname = 'update_shopping_carts_updated_at') THEN
...
BEGIN
  ...
  WHERE product_id = product_uuid AND is_visible = TRUE;
...
-- 迁移完成
-- ============================================
SELECT '津门老字号文创商城数据库表创建完成! (Neon PostgreSQL)' as message;
