...
// 生成额外的mock作品
function generateAdditionalMockWorks(count, startId) {
  const works = [];
  for (let i = 0; i < count; i++) {
    works.push(generateMockWork(startId + i));
  }
  return works;
}
...
async function main() {
  try {
    ...
    // 生成105个额外的作品，从ID 196开始
    const additionalWorks = generateAdditionalMockWorks(105, 196);
    
    // 将额外的作品转换为TypeScript格式
    const additionalWorksString = additionalWorks.map(work => JSON.stringify(work, null, 2)).join(',\n');
    
    // 找到数组声明的开始和结束位置
    const arrayDeclStart = existingContent.indexOf('export const mockWorks: Work[] = [');
    if (arrayDeclStart === -1) {
    ...
    // 数组内容的开始位置（跳过声明和'['）
    const arrayContentStart = arrayDeclStart + 'export const mockWorks: Work[] = ['.length;
    
    // 找到数组结束位置
    let arrayEnd = existingContent.lastIndexOf(']');
    if (arrayEnd === -1) {
      console.error('未找到数组结束标记！');
      return;
    }
    
    // 确保数组结束位置在数组内容开始位置之后
    if (arrayEnd < arrayContentStart) {
      console.error('数组结束位置在数组内容开始位置之前！');
      return;
    }
    
    // 获取数组内容（不包括开始和结束括号）
    const arrayContent = existingContent.slice(arrayContentStart, arrayEnd);
    
    // 检查数组是否为空
    const isArrayEmpty = arrayContent.trim() === '';
    
    // 构建新的数组内容
    let newArrayContent;
    if (isArrayEmpty) {
      // 空数组，直接添加新作品
      newArrayContent = `${additionalWorksString}`;
    } else {
      // 非空数组，在最后一个作品后添加新作品
      newArrayContent = `${arrayContent}\n,\n${additionalWorksString}`;
    }
    
    // 构建新的文件内容
    const newContent = existingContent.slice(0, arrayContentStart) + 
                      newArrayContent + 
    ...
    console.log(`成功生成并添加了${additionalWorks.length}个额外的mock作品！`);
    console.log(`现在总共有${195 + additionalWorks.length}个作品。`);
  } catch (error) {
