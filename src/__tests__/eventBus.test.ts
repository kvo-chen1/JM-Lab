...
describe('Event Bus Basic Functionality', () => {
  ...
  test('should subscribe to and publish events', () => {
    ...
    // 订阅事件
    const unsubscribe = eventBus.on('auth:login', mockCallback)
    
    // 发布事件
    const testData = { userId: '123', user: { username: 'testuser', email: 'test@example.com' } }
    eventBus.emit('auth:login', testData)
  ...
  // 测试事件优先级（EventEmitter 不直接支持优先级，使用顺序执行）
  test('should handle multiple listeners in order', () => {
    const order: number[] = []
    
    // 订阅多个事件
    eventBus.on('作品:创建', () => order.push(1))
    eventBus.on('作品:创建', () => order.push(2))
    eventBus.on('作品:创建', () => order.push(3))
    
    // 发布事件
    eventBus.emit('作品:创建', { workId: '123', work: { title: 'Test Work' } })
  ...
  test('should handle multiple listeners for the same event', () => {
    ...
    // 订阅同一个事件的多个监听器
    eventBus.on('作品:发布', mockCallback1)
    eventBus.on('作品:发布', mockCallback2)
    eventBus.on('作品:发布', mockCallback3)
    
    // 发布事件
    const testData = { workId: '123', work: { title: 'Test Work' } }
    eventBus.emit('作品:发布', testData)
  ...
  test('should clear all listeners', () => {
    ...
    // 订阅不同类型的事件
    eventBus.on('auth:login', mockCallback1)
    eventBus.on('作品:创建', mockCallback2)
    
    // 清理所有事件监听器
    eventBus.removeAllListeners()
    
    // 发布事件，验证回调函数不再被调用
    eventBus.emit('auth:login', { userId: '123', user: { username: 'testuser' } })
    eventBus.emit('作品:创建', { workId: '123', work: { title: 'Test Work' } })
  ...
  test('should get correct listener count', () => {
    ...
    // 订阅一个事件
    eventBus.on('auth:login', jest.fn())
    expect(eventBus.listenerCount('auth:login')).toBe(1)
    
    // 订阅另一个相同类型的事件
    eventBus.on('auth:login', jest.fn())
    expect(eventBus.listenerCount('auth:login')).toBe(2)
    
    // 订阅不同类型的事件
    eventBus.on('auth:logout', jest.fn())
    expect(eventBus.listenerCount('auth:login')).toBe(2)
  ...
  test('should get registered event types', () => {
    ...
    eventBus.on('auth:login', jest.fn())
    eventBus.on('auth:logout', jest.fn())
    eventBus.on('作品:创建', jest.fn())
    
    // 验证注册的事件类型
    const registeredEvents = eventBus.eventNames()
...
describe('Event Bus Cross-Module Communication', () => {
  ...
  test('should communicate between different modules', () => {
    ...
    eventBus.on('作品:创建', moduleA_Callback)
    
    // 模拟模块B：订阅作品发布事件
    const moduleB_Callback = jest.fn()
    eventBus.on('作品:发布', moduleB_Callback)
    
    // 模拟模块C：订阅作品审核事件
    const moduleC_Callback = jest.fn()
    eventBus.on('作品:审核', moduleC_Callback)
  ...
  test('should handle data refresh events', () => {
    ...
    // 订阅数据刷新事件
    eventBus.on('数据:刷新', mockCallback)
    ...
    eventBus.emit('数据:刷新', refreshData1)
    eventBus.emit('数据:刷新', refreshData2)
    eventBus.emit('数据:刷新', refreshData3)
...
describe('Event Bus Edge Cases', () => {
  ...
  test('should handle unsubscribe of non-existent listeners', () => {
    ...
    // 取消订阅一个不存在的事件类型（不应该抛出错误）
    expect(() => {
      eventBus.off('auth:logout', handler)
    }).not.toThrow()
  ...
  test('should handle publishing non-existent event types', () => {
    ...
    expect(() => {
      ...
      eventBus.emit('non-existent-event' as any, { data: 'test' })
    }).not.toThrow()
  })
  ...
  test('should handle a large number of listeners', () => {
    ...
    mockCallbacks.forEach(callback => {
      eventBus.on('作品:创建', callback)
    })
    
    // 验证监听器数量
    expect(eventBus.listenerCount('作品:创建')).toBe(callbackCount)
    
    // 发布事件
    const testData = { workId: '123', work: { title: 'Test Work' } }
    eventBus.emit('作品:创建', testData)
