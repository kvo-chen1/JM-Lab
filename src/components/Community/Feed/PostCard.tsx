  ...
  return (
    ...
    >
      <div className="flex">
        ...
        <div className="flex-1 p-3">
          ...
          <div className="flex items-center gap-2 mb-2 text-xs">
            <TianjinAvatar size="xs" src="" alt="User" className="w-6 h-6" />
      ...
      <div className={`p-4 rounded-b-xl ${isDark ? 'bg-gray-800/50' : 'bg-gray-50'}`}>
        ...
        {thread.comments && thread.comments.length > 0 && (
          <div className="mt-3 pt-3 border-t border-gray-700/50 dark:border-gray-700/50">
            ...
            <div className={`p-2 rounded-lg bg-gray-700/20 dark:bg-gray-700/30`}>
              <div className="flex items-start gap-2">
                <TianjinAvatar size="xs" src="" alt="Commenter" className="w-5 h-5 mt-0.5" />
                <div className="flex-1">
                  <div className="text-xs font-medium ${isDark ? 'text-gray-300' : 'text-gray-900'}">
                    u/{thread.comments[0].user}
                  </div>
      ...
      <div className={`flex items-center gap-3 px-3 py-3 ${isDark ? 'border-t border-gray-700' : 'border-t border-gray-200'}`}>
          <button className="flex items-center gap-1.5 p-2 rounded-lg text-xs font-medium hover:bg-gray-200/20 transition-colors ${isDark ? 'text-gray-400 hover:text-gray-300' : 'text-gray-600 hover:text-gray-900'}">
              <i className="fas fa-comment-alt"></i>
              <span>{thread.replies.length} 评论</span>
          </button>
          <button className="flex items-center gap-1.5 p-2 rounded-lg text-xs font-medium hover:bg-gray-200/20 transition-colors ${isDark ? 'text-gray-400 hover:text-gray-300' : 'text-gray-600 hover:text-gray-900'}">
