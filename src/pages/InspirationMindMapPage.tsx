...
const InspirationMindMapPage: React.FC = () => {
  ...
  // 脉络列表状态已移除 - 现在直接显示所有脉络的节点

  // 获取选中的节点
  const selectedNode = selectedNodeId ? nodes.find(n => n.id === selectedNodeId) || null : null;

  // 初始化：加载用户所有脉络的节点（合并视图）
  ...
  useEffect(() => {
    if (!mindMap && user && !isLoading && !hasInitialized.current) {
      ...
      (async () => {
        try {
          console.log('[InspirationMindMapPage] Loading all nodes for user:', user.id);
          await loadAllUserNodes(user.id);
        } catch (err: any) {
          console.error('[InspirationMindMapPage] Error loading all nodes:', err);
          // 如果没有脉络，创建新的
          if (err.message === 'NO_MINDMAP') {
            await createMindMap(user.id, '我的创作脉络');
          }
        }
      })();
    }
  }, [mindMap, user, createMindMap, loadAllUserNodes, isLoading]);
