...
const MockupPanel: React.FC = () => {
  ...
  const handleApplyMockup = () => {
    ...
    setTimeout(() => {
      updateState({
        aiExplanation: `已应用${selectedMockup.name}模型模板，预览质量为${previewQuality === 'low' ? '低' : previewQuality === 'medium' ? '中' : '高'}`,
        showAIReview: true
      });
      setIsApplying(false);
    }, 2000);
    ...
    <div className="h-full flex flex-col p-4 overflow-y-auto">
          ...
          return (
            ...
            >
              ...
              <div className="p-2 bg-gray-100 dark:bg-gray-900">
                ...
                <p className="text-[9px] opacity-70 truncate">{mockup.description}</p>
              </div>
            </motion.button>
          );
        })}
      ...
      {selectedMockup && (
        <div className={`rounded-lg border p-3 mb-4 ${isDark ? 'bg-gray-900 border-gray-800' : 'bg-white border-gray-200'}`}>
          <h4 className="text-sm font-medium mb-3">预览设置</h4>
          ...
          <div className="rounded-lg overflow-hidden mb-3">
            <img
              ...
              onError={(e) => {
                // 图片加载失败时显示占位符
                const target = e.target as HTMLImageElement;
                target.src = `https://via.placeholder.com/600x600?text=${encodeURIComponent(`${selectedMockup.name}预览`)}`;
              }}
            />
          </div>
          ...
          {/* 预览质量设置 */}
          <div className="mb-3">
            <label className="block text-xs font-medium mb-2">预览质量</label>
            <div className="flex gap-2">
              {(['low', 'medium', 'high'] as const).map((quality) => (
                <motion.button
                  key={quality}
                  onClick={() => setPreviewQuality(quality)}
                  className={`flex-1 px-3 py-2 rounded-lg text-xs transition-all ${isDark 
      ...
      <div className="mt-auto">
        ...
        <p className="text-xs text-center opacity-50 mt-2">选择模型模板以应用</p>
      </div>
    </div>
  );
};
