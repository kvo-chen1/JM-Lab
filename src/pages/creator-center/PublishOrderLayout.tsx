...
import PublishOrderPage from './components/PublishOrderPage';

export type CreatorTab = 'overview' | 'promotion' | 'monetization' | 'inspiration' | 'analytics';

const PublishOrderLayout: React.FC = () => {
  const { isDark } = useTheme();
  const { user } = useAuth();
  const navigate = useNavigate();
  // 使用扩展类型来支持 'order-monetization'
...
export default PublishOrderLayout;
