...
async function getBookmarkedCommunityPosts(
  ...
  try {
    ...
    try {
      ...
      console.error('[getBookmarkedCommunityPosts] Error accessing bookmarks:', err);
      return [];
    }
...
export async function getUserCollectionStats(): Promise<UserCollectionStats> {
  try {
    ...
    try {
      ...
      console.error('[getUserCollectionStats] Error accessing bookmarks:', error);
    }
  ...
  try {
    ...
    if (type === CollectionType.SQUARE_WORK) {
      ...
      try {
        ...
        console.error('[toggleBookmark] Error accessing bookmarks:', error);
        toast.error('收藏操作失败');
        return false;
      }
