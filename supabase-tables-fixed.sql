...
CREATE POLICY "Replies are viewable by community members" ON replies FOR SELECT USING (
    EXISTS (
        SELECT 1 FROM community_members 
        JOIN posts ON posts.community_id = community_members.community_id
        WHERE posts.id = replies.post_id AND community_members.user_id = auth.uid()
    )
);
...
CREATE POLICY "Users can manage their own favorites" ON favorites FOR INSERT WITH CHECK (auth.uid() = user_id);
CREATE POLICY "Users can manage their own favorites" ON favorites FOR DELETE USING (auth.uid() = user_id);

-- 关注表策略
ALTER TABLE follows ENABLE ROW LEVEL SECURITY;
CREATE POLICY "Users can view their own follows" ON follows FOR SELECT USING (follower_id = auth.uid() OR following_id = auth.uid());
...
CREATE POLICY "Membership benefits are viewable by everyone" ON membership_benefits FOR SELECT USING (true);

-- 计划帖子表策略
ALTER TABLE scheduled_posts ENABLE ROW LEVEL SECURITY;
CREATE POLICY "Users can view their own scheduled posts" ON scheduled_posts FOR SELECT USING (user_id = auth.uid());
CREATE POLICY "Users can create scheduled posts" ON scheduled_posts FOR INSERT WITH CHECK (user_id = auth.uid());
CREATE POLICY "Users can update their own scheduled posts" ON scheduled_posts FOR UPDATE USING (user_id = auth.uid());
CREATE POLICY "Users can delete their own scheduled posts" ON scheduled_posts FOR DELETE USING (user_id = auth.uid());

-- 社区公告表策略
ALTER TABLE community_announcements ENABLE ROW LEVEL SECURITY;
CREATE POLICY "Community announcements are viewable by members" ON community_announcements FOR SELECT USING (
    EXISTS (
        SELECT 1 FROM community_members 
        WHERE community_id = community_announcements.community_id AND user_id = auth.uid()
    )
);
CREATE POLICY "Community admins can manage announcements" ON community_announcements FOR INSERT WITH CHECK (
    EXISTS (
        SELECT 1 FROM community_members 
        WHERE community_id = community_announcements.community_id AND user_id = auth.uid() AND role IN ('owner', 'admin')
    )
);
CREATE POLICY "Community admins can manage announcements" ON community_announcements FOR UPDATE WITH CHECK (
    EXISTS (
        SELECT 1 FROM community_members 
        WHERE community_id = community_announcements.community_id AND user_id = auth.uid() AND role IN ('owner', 'admin')
    )
);
CREATE POLICY "Community admins can manage announcements" ON community_announcements FOR DELETE WITH CHECK (
    EXISTS (
        SELECT 1 FROM community_members 
        WHERE community_id = community_announcements.community_id AND user_id = auth.uid() AND role IN ('owner', 'admin')
    )
