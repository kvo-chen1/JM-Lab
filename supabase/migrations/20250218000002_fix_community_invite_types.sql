-- 修复社群邀请系统表类型不匹配问题
-- 将 UUID 改为 VARCHAR(50) 以匹配 communities 表的 id 类型

-- ============================================
-- 1. 删除已创建的表（如果存在）
-- ============================================
DROP TABLE IF EXISTS invitation_reports CASCADE;
DROP TABLE IF EXISTS user_invite_rate_limits CASCADE;
DROP TABLE IF EXISTS community_invite_settings CASCADE;
DROP TABLE IF EXISTS community_invitation_history CASCADE;
...
CREATE TABLE IF NOT EXISTS community_invite_settings (
    id UUID PRIMARY KEY DEFAULT gen_random_uuid(),
    community_id VARCHAR(50) NOT NULL UNIQUE REFERENCES communities(id) ON DELETE CASCADE,
...
-- 获取社群邀请统计
CREATE OR REPLACE FUNCTION get_community_invite_stats(p_community_id VARCHAR(50))
RETURNS TABLE (
