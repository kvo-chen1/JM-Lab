...
-- 1. 修复 get_user_points_stats 函数（修复类型转换问题）
...
-- 2. 修复 check_points_limit 函数（修复时间间隔语法）
...
BEGIN
  ...
  v_period_start := CASE 
    ...
    WHEN p_period_type = 'weekly' THEN DATE_TRUNC('week', CURRENT_TIMESTAMP)::DATE
    WHEN p_period_type = 'monthly' THEN DATE_TRUNC('month', CURRENT_TIMESTAMP)::DATE
    WHEN p_period_type = 'yearly' THEN DATE_TRUNC('year', CURRENT_TIMESTAMP)::DATE
    ELSE CURRENT_DATE
  END;
