...
BEGIN
    ...
    SELECT jsonb_agg(
        jsonb_build_object(
            'submission_id', es.id,
            'rank', row_number() OVER (ORDER BY COALESCE(ss.avg_score, 0) DESC, es.created_at ASC),
            'title', es.title,
            'creator_id', es.user_id,
            'creator_name', COALESCE(pu.username, u.raw_user_meta_data->>'username'),
            ...
            'submitted_at', es.created_at
        )
        ORDER BY COALESCE(ss.avg_score, 0) DESC, es.created_at ASC
    )
    INTO v_ranking_data
...
BEGIN
    ...
    FOR v_participant IN
        ...
        WHERE es.event_id = p_event_id
    LOOP
        -- 插入通知记录（这里使用 notifications 表，如果不存在需要提前创建）
