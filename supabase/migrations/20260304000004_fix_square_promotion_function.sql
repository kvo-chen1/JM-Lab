-- ==========================================================================
-- 修复 get_square_works_with_promotion 函数
-- ==========================================================================
...
BEGIN
  ...
  SELECT
    ...
    -- 处理 bigint 类型的 created_at (假设是毫秒时间戳)
  ...
  SELECT
    ...
    pw.created_at::TIMESTAMPTZ,
    TRUE as is_promoted,
    pw.id::UUID as promoted_work_id,
    pw.package_type::TEXT,
    pw.promotion_weight::DECIMAL(3,2)
...
-- 添加注释
COMMENT ON FUNCTION get_square_works_with_promotion IS '获取广场作品列表，包含推广作品（修复版）';
